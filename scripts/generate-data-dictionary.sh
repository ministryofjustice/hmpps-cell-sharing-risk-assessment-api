#!/usr/bin/env bash
#
# Exports the CSRA schema as a flat CSV data dictionary.
#
# The descriptions come from the COMMENT ON statements in db/migration/V14__schema_comments.sql, so this
...
#   DB_HOST (localhost) DB_PORT (5432) DB_NAME (cell_sharing_risk_assessment) DB_USER (cell_sharing_risk_assessment)
#   DB_PASSWORD (cell_sharing_risk_assessment) DB_SCHEMA (public)
...
DB_NAME="${DB_NAME:-cell_sharing_risk_assessment}"
DB_USER="${DB_USER:-cell_sharing_risk_assessment}"
DB_PASSWORD="${DB_PASSWORD:-cell_sharing_risk_assessment}"
...
-- query fans out, and columns are emitted once per matching constraint. Every name in this schema is
-- table-prefixed today, so nothing collides yet; the bug fires silently the first time a migration
-- adds a duplicate name. It has already happened elsewhere: hmpps-incentives-api had
-- incentive_level_fkey on two tables and published 48 rows instead of 42. pg_constraint carries
-- conrelid, the owning relation, so it cannot collide. Do not put this back on information_schema.
LEFT JOIN (
