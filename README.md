...
[ministryofjustice.github.io/hmpps-cell-sharing-risk-assessment-api/schema-spy-report](https://ministryofjustice.github.io/hmpps-cell-sharing-risk-assessment-api/schema-spy-report/).

It shows every table and column, with types, nullability, primary and foreign keys, and ER diagrams.
Share this rather than a hand-written description when explaining the schema — to the Analytical
Platform team, for the Data Hub transition, or when working out what a subject access request covers.

The report is generated by SchemaSpy against a database built by Flyway, so it cannot drift from the
migrations. To regenerate it locally:
...
docker run --rm --network host -v /tmp/schemaspy:/output schemaspy/schemaspy:6.2.4 \
  -t pgsql -host localhost -port 5432 -db cell_sharing_risk_assessment -s public \
  -u cell_sharing_risk_assessment -p cell_sharing_risk_assessment -vizjs
```
...
A CSV data dictionary and reference data export for the MOJ Data Catalogue follows in MAPA-296.
