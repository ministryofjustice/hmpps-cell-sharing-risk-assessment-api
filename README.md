...
| `reference-data.csv` | The enum lookups, including the legacy NOMIS codes on `csra_review_nomis`. Every code in this schema resolves in Kotlin — there are no reference tables — so without this a consumer sees a `varchar` with no idea which values are legal |
